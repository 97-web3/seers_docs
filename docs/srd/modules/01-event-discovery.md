...
- FR-ED-003（MVP）：特色/热门选择应由 `GET /api/markets/featured` 提供；当该能力不可用时，客户端可以降级为展示当前列表的前 3 个盘口（占位策略）。
...
- FR-ED-008（MVP）：完整列表应保持后端返回的默认顺序；如存在置顶权重，则置顶权重更高的盘口应优先展示。
- FR-ED-009（未来）：热门度得分可用于排序；若启用，建议公式为：`参与人数 * 0.4 + 规范化投注额 * 0.4 + 过去24小时增量 * 0.2`。
- FR-ED-010：盘口卡片应支持至少两种显示类型：正面交锋（head-to-head）赛事和冠军/未来赛（futures）赛事。
- FR-ED-011（MVP）：盘口卡片应显示投注选项预览：
  - 如果选项数量 ≤ 4，则全部显示。
  - 如果选项数量 ≥ 5，则显示前 4 个以及一个“更多投注项 +N”提示（N=剩余数量）。
- FR-ED-012：领先选项（份额最高）的进度指示器应为红色；份额最低的应为绿色；中间份额的应为蓝色。
- FR-ED-013（MVP）：由创建者锁定/标记为“庄家持仓”的选项应有明显的锁定标识与文案提示，并且不可点击下注。
- FR-ED-014：点击盘口卡片的非选项区域应导航至盘口详情视图。
- FR-ED-015：点击卡片上的投注选项预览应启动该选项的投注流程，除非该选项已锁定；锁定的选项不可操作。
- FR-ED-016（MVP）：系统可以展示“搜索预测市场”的输入框；若搜索能力未启用，则该输入框不得对结果产生筛选影响，并应避免造成“已启用搜索”的误导（文案/占位策略待定）。
...
## 业务规则
- BR-ED-001（MVP）：特色选择以 `GET /api/markets/featured` 为准；降级策略为“列表前 3 条”。
- BR-ED-002（未来）：完整列表排序优先级可为：置顶权重 > 热门度得分 > 创建时间。
- BR-ED-003（未来）：热门度得分权重应如 FR-ED-009 中定义，为 0.4 / 0.4 / 0.2。
- BR-ED-004：“活跃”盘口是指状态为 `ACTIVE` 的盘口（参见`modules/06-market-states-and-settlement.md`）。
- BR-ED-005（MVP）：如果选项数量 ≤ 4，则不显示“更多投注项 +N”。
- BR-ED-006（MVP）：“所有体育/全部体育”视图仅聚合已配置的体育类别下的市场；不得混入非体育领域市场。
...
## 卡片渲染逻辑
- BL-ED-003（MVP）：卡片的选项预览顺序按选项份额降序排列，最多显示前 4 个。
...
- AC-ED-003（MVP）：当存在置顶权重时，置顶权重更高的盘口出现在列表前部；其余顺序保持后端默认返回顺序。
- AC-ED-004（MVP）：对于具有 5 个或更多选项的盘口，恰好显示 4 个选项加上“更多投注项 +N”。
